  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <ToggleSwitch v-if="form.provider_key !== 'xunhupay'" :label="t('admin.settings.payment.refundEnabled')" :checked="form.refund_enabled" @toggle="form.refund_enabled = !form.refund_enabled; if (!form.refund_enabled) form.allow_user_refund = false" />
        <ToggleSwitch v-if="form.provider_key !== 'xunhupay' && form.refund_enabled" :label="t('admin.settings.payment.allowUserRefund')" :checked="form.allow_user_refund" @toggle="form.allow_user_refund = !form.allow_user_refund" />
...
function loadProvider(provider: ProviderInstance) {
  ...
  form.refund_enabled = provider.provider_key === 'xunhupay' ? false : provider.refund_enabled
  form.allow_user_refund = provider.provider_key === 'xunhupay' ? false : provider.allow_user_refund
  clearConfig()
