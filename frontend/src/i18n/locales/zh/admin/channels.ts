export default {
    ...
    riskControl: {
      ...
      classifierPolicyRevision: '分类器策略版本',
      classifierPolicyRevisionHint: '必须与审核适配器就绪接口返回的版本完全一致。',
      ...
      queueDelay: '排队 {ms} ms',
      allGroups: '全部分组',
      allGroupsHint: '当前默认审核除豁免分组外的全部请求',
      defaultOnGroups: '默认审核',
      selectedGroupsHint: '旧版白名单模式：仅审核下方选中的分组。',
      groupScope: '审计分组',
      groupScopeHint: '默认审核除显式豁免外的所有现有、新建及未分组请求；切换到指定分组后沿用旧版白名单语义。',
      selectedGroups: '指定分组',
      excludedGroups: '豁免分组',
      excludedGroupsHint: '按名称、平台或 ID 搜索并选择豁免；保存前核对名称和 ID，同名分组不会联动。',
      ...
      runtimeNoExcludedGroups: '当前无豁免分组',
      runtimeLegacyAllowlist: '当前运行旧版分组白名单模式',
