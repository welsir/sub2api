 ...
 * [POS]: Startup configuration boundary for the standalone moderation adapter.
 *
 * [PROTOCOL]:
...
function requiredText(env: Env, name: string): string {
...
function repeatedlyDecodedPath(rawPath: string, settingName: string): string {
  let decoded = rawPath;
...
function rawAbsoluteUrlPath(rawUrl: string): string {
...
function backendProvider(env: Env): BackendProvider {
