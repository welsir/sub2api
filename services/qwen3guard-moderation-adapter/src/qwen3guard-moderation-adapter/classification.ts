 ...
 * [POS]: Taxonomy and parser boundary between Qwen inference and Sub2API.
 *
 * [PROTOCOL]:
...
export type EvaluatedCategory = (typeof evaluatedCategories)[number];
export type QwenSafetyLabel = "Safe" | "Controversial" | "Unsafe";

export interface MappedClassification {
  label: QwenSafetyLabel;
  sourceCategoryCount: number;
  mappedCategories: EvaluatedCategory[];
  categoryMapping: "none" | "mapped" | "fallback" | "mapped_with_fallback";
  flagged: boolean;
  policyValue: 0 | 0.5 | 1;
  categories: Record<EvaluatedCategory, boolean>;
  categoryScores: Record<EvaluatedCategory, number>;
...
function labelFrom(value: unknown): QwenSafetyLabel {
...
function parseJsonOutput(output: string): ParsedClassification | undefined {
  ...
  return {
    label: labelFrom(record.safety ?? record.label),
    categories: categoryList(record.categories ?? record.category)
  };
}

function parseTextOutput(output: string): ParsedClassification {
...
function mappedCategories(rawCategories: string[]): {
  mapped: Set<EvaluatedCategory>;
  unknownCount: number;
} {
  const mapped = new Set<EvaluatedCategory>();
  let unknownCount = 0;
...
export function parseAndMapClassification(output: string): MappedClassification {
  ...
  ) as Record<EvaluatedCategory, boolean>;
