...
const (
	...
	EdgeGroup = "group"
	// EdgeSelectedGroups holds the string denoting the selected_groups edge name in mutations.
	EdgeSelectedGroups = "selected_groups"
	...
	// SelectedGroupsTable is the table that holds the selected_groups relation/edge. The primary key declared below.
	SelectedGroupsTable = "api_key_groups"
	// SelectedGroupsInverseTable is the table name for the Group entity.
	// It exists in this package in order to avoid circular dependency with the "group" package.
	SelectedGroupsInverseTable = "groups"
...
var (
	// SelectedGroupsPrimaryKey and SelectedGroupsColumn2 are the table columns denoting the
	// primary key for the selected_groups relation (M2M).
	SelectedGroupsPrimaryKey = []string{"api_key_id", "group_id"}
)
...
// BySelectedGroupsCount orders the results by selected_groups count.
func BySelectedGroupsCount(opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborsCount(s, newSelectedGroupsStep(), opts...)
	}
}

// BySelectedGroups orders the results by selected_groups terms.
func BySelectedGroups(term sql.OrderTerm, terms ...sql.OrderTerm) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newSelectedGroupsStep(), append([]sql.OrderTerm{term}, terms...)...)
	}
}
...
func newSelectedGroupsStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(SelectedGroupsInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.M2M, false, SelectedGroupsTable, SelectedGroupsPrimaryKey...),
	)
}
