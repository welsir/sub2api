...
// HasSelectedGroups applies the HasEdge predicate on the "selected_groups" edge.
func HasSelectedGroups() predicate.APIKey {
	return predicate.APIKey(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasSelectedGroupsWith applies the HasEdge predicate on the "selected_groups" edge with a given conditions (other predicates).
func HasSelectedGroupsWith(preds ...predicate.Group) predicate.APIKey {
	return predicate.APIKey(func(s *sql.Selector) {
		step := newSelectedGroupsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
