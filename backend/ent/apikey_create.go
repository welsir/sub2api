...
func (_c *APIKeyCreate) AddSelectedGroupIDs(ids ...int64) *APIKeyCreate {
	_c.mutation.AddSelectedGroupIDs(ids...)
	return _c
}

// AddSelectedGroups adds the "selected_groups" edges to the Group entity.
func (_c *APIKeyCreate) AddSelectedGroups(v ...*Group) *APIKeyCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddSelectedGroupIDs(ids...)
}
...
func (_c *APIKeyCreate) createSpec() (*APIKey, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.SelectedGroupsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
