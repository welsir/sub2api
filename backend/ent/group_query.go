...
type GroupQuery struct {
	...
	withSelectedAPIKeys   *APIKeyQuery
...
// QuerySelectedAPIKeys chains the current query on the "selected_api_keys" edge.
func (_q *GroupQuery) QuerySelectedAPIKeys() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, selector),
			sqlgraph.To(apikey.Table, apikey.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, group.SelectedAPIKeysTable, group.SelectedAPIKeysPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withSelectedAPIKeys:   _q.withSelectedAPIKeys.Clone(),
...
// WithSelectedAPIKeys tells the query-builder to eager-load the nodes that are connected to
// the "selected_api_keys" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithSelectedAPIKeys(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSelectedAPIKeys = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSelectedAPIKeys; query != nil {
		if err := _q.loadSelectedAPIKeys(ctx, query, nodes,
			func(n *Group) { n.Edges.SelectedAPIKeys = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.SelectedAPIKeys = append(n.Edges.SelectedAPIKeys, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadSelectedAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Group)
	nids := make(map[int64]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.SelectedAPIKeysTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(group.SelectedAPIKeysPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.SelectedAPIKeysPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.SelectedAPIKeysPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "selected_api_keys" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
