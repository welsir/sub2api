...
// QuerySelectedGroups chains the current query on the "selected_groups" edge.
func (_q *APIKeyQuery) QuerySelectedGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(apikey.Table, apikey.FieldID, selector),
			sqlgraph.To(group.Table, group.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, apikey.SelectedGroupsTable, apikey.SelectedGroupsPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withSelectedGroups: _q.withSelectedGroups.Clone(),
...
// WithSelectedGroups tells the query-builder to eager-load the nodes that are connected to
// the "selected_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithSelectedGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSelectedGroups = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withSelectedGroups; query != nil {
		if err := _q.loadSelectedGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.SelectedGroups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.SelectedGroups = append(n.Edges.SelectedGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadSelectedGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.SelectedGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.SelectedGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.SelectedGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.SelectedGroupsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "selected_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
