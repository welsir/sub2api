...
type UserQuery struct {
	...
	withActivationJourney     *UserActivationJourneyQuery
...
// QueryActivationJourney chains the current query on the "activation_journey" edge.
func (_q *UserQuery) QueryActivationJourney() *UserActivationJourneyQuery {
	query := (&UserActivationJourneyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withActivationJourney:     _q.withActivationJourney.Clone(),
...
// WithActivationJourney tells the query-builder to eager-load the nodes that are connected to
// the "activation_journey" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithActivationJourney(opts ...func(*UserActivationJourneyQuery)) *UserQuery {
	query := (&UserActivationJourneyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withActivationJourney = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withActivationJourney; query != nil {
		if err := _q.loadActivationJourney(ctx, query, nodes, nil,
			func(n *User, e *UserActivationJourney) { n.Edges.ActivationJourney = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadActivationJourney(ctx context.Context, query *UserActivationJourneyQuery, nodes []*User, init func(*User), assign func(*User, *UserActivationJourney)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(useractivationjourney.FieldUserID)
	}
	query.Where(predicate.UserActivationJourney(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ActivationJourneyColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
