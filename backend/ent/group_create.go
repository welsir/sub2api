...
func (_c *GroupCreate) AddSelectedAPIKeyIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddSelectedAPIKeyIDs(ids...)
	return _c
}

// AddSelectedAPIKeys adds the "selected_api_keys" edges to the APIKey entity.
func (_c *GroupCreate) AddSelectedAPIKeys(v ...*APIKey) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddSelectedAPIKeyIDs(ids...)
}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.SelectedAPIKeysIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
