...
func TestAccountTestService_OpenAIResponsesUsesRequestedPrompt(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
`))
	...
	err := svc.testOpenAIAccountConnection(ctx, account, "gpt-5.6-sol", "benchmark-context", "")
	require.NoError(t, err)
	require.Len(t, upstream.requests, 1)
	body, err := io.ReadAll(upstream.requests[0].Body)
	require.NoError(t, err)
	require.Equal(t, "benchmark-context", gjson.GetBytes(body, "input.0.content.0.text").String())
}
