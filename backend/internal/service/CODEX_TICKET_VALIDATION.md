...
`harvested` now means the collection response passed admission. `injected` records the account, requested model, length, and short state/route digest; it never records the state or proxy credentials. Injection is not a guarantee that a later upstream response will retain the model. Strict mode is request admission only: an unavailable or invalid collected state blocks the formal request before forwarding. Once admitted, formal responses use the normal streaming parser without whole-response buffering or a completion-time model gate. Collection probes still require a complete matching-model response before accepting a state. Formal response model metadata and upstream failures remain observable through the normal response and billing paths; an already-streamed response cannot be withdrawn. Compaction retains its existing protocol validation instead of requiring the generation response model/state shape. Matching upstream metadata is still not independent proof of model internals or reasoning quality.
