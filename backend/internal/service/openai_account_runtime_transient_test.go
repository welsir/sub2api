...
func TestHandleOpenAITransientError_HardDisableStillBlocksWholeAccount(t *testing.T) {
	...
	require.False(t, svc.isOpenAIAccountRequestRuntimeBlocked(context.Background(), account, "gpt-5.5", false))
	require.False(t, svc.isOpenAIAccountRequestRuntimeBlocked(context.Background(), account, "gpt-5.6-sol", false))
	require.False(t, svc.isOpenAIAccountRuntimeBlocked(account))
}
