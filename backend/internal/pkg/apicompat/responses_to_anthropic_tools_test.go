...
func TestResponsesToAnthropic_MixedToolsDropUnsupportedWebSearch(t *testing.T) {
	tools := convertResponsesToAnthropicTools([]ResponsesTool{
		{
			Type:       "function",
			Name:       "read_file",
	...
	assert.JSONEq(t, `{"type":"object","properties":{"input":{"type":"string","description":"Free-form input passed verbatim to the tool."}},"required":["input"]}`, string(tools[1].InputSchema))
}
